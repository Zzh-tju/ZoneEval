...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        cls_dets, cls_gts, cls_gts_ignore = get_cls_results(
            det_results, annotations, data, i)
    ...
    if scale_ranges is not None:
        ...
        for cls_result in eval_results:
            apss.append(cls_result['ap'])
            if cls_result['num_gts'] > 0:
                aps.append(cls_result['ap'])
        cls_aps = np.array(apss)
        mean_ap = cls_aps.mean().item() if aps else 0.0

    mean_ar = print_map_summary(
        mean_ap, eval_results, dataset, area_ranges, logger=logger)

    return mean_ap, mean_ar, cls_aps, eval_results

def eval_zone_map(det_results,
             annotations,
             data,
             ri,
             rj,
    ...
    for i in range(num_classes):
        # get gt and det bboxes of this class
        cls_dets, cls_gts, cls_gts_ignore = get_cls_zone_results(
            det_results, annotations, data, i, ri, rj)
        # choose proper function according to datasets to compute tp and fp
        if tpfp_fn is None:
            if dataset in ['det', 'vid']:
                tpfp_fn = tpfp_imagenet
...
def print_map_summary(mean_ap,
    ...
    for i in range(num_scales):
        ...
        for j in range(num_classes):
            mean_ar = recalls[i, j] + mean_ar
            row_data = [
                label_names[j], num_gts[i, j], results[j]['num_dets'],
                f'{recalls[i, j]:.3f}', f'{aps[i, j]:.3f}'
            ]
            table_data.append(row_data)
        mean_ar = mean_ar / num_classes
        table_data.append(['mean value', '', '', f'{mean_ar:.3f}', f'{mean_ap[i]:.3f}'])
