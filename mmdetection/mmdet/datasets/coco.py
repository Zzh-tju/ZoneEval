...
class CocoDataset(CustomDataset):
    ...
    def evaluate_zone_det_segm(self,
        ...
        return eval_results, ap, class_ap
    ...
    def zone_evaluate(self,
        ...
        self.cat_ids = coco_gt.get_cat_ids(cat_names=self.CLASSES)
        result_files, tmp_dir = self.format_results(results, jsonfile_prefix)
        eval_results, metrics, class_ap = self.evaluate_zone_det_segm(results, result_files, coco_gt,
                                              metrics, logger, classwise,
                                              proposal_nums, iou_thrs,
                                              metric_items, ri, rj)
        if tmp_dir is not None:
            tmp_dir.cleanup()
        return eval_results, np.array(metrics)*100, class_ap, None
