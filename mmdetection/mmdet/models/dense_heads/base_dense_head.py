...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def get_zone_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            results = self._get_zone_bboxes_single(cls_score_list, bbox_pred_list,
                                              score_factor_list, mlvl_priors,
                                              img_meta, cfg, ri, rj, rescale, with_nms,
                                              **kwargs)
            result_list.append(results)
        return result_list
    ...
    def _get_zone_bboxes_single(self,
        ...
        return self._zone_bbox_post_process(mlvl_scores, mlvl_labels, mlvl_bboxes,
                                       img_meta['scale_factor'], cfg, ri, rj, img_w, img_h, rescale,
                                       with_nms, mlvl_score_factors, **kwargs)
    ...
    def _zone_bbox_post_process(self,
        ...
        if rescale:
            mlvl_bboxes /= mlvl_bboxes.new_tensor(scale_factor)
            img_w = img_w / scale_factor[0]
            img_h = img_h / scale_factor[1]
        mlvl_scores = torch.cat(mlvl_scores)
        mlvl_labels = torch.cat(mlvl_labels)
    ...
    def simple_zone_test(self, feats, img_metas, ri, rj, rescale=False):
        """Test function without test-time augmentation.
        ...
        Returns:
            list[tuple[Tensor, Tensor]]: Each item in result_list is 2-tuple.
                ...
                with shape (n, ).
        """
        return self.simple_zone_test_bboxes(feats, img_metas, ri, rj, rescale=rescale)
