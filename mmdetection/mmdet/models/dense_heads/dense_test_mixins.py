...
class BBoxTestMixin(object):
    ...
    def simple_zone_test_bboxes(self, feats, img_metas, ri, rj, rescale=False):
        """Test det bboxes without test-time augmentation, can be applied in
        DenseHead except for ``RPNHead`` and its variants, e.g., ``GARPNHead``,
        etc.
        ...
        """
        outs = self.forward(feats)
        results_list = self.get_zone_bboxes(
            *outs, img_metas=img_metas, ri=ri, rj=rj, rescale=rescale)
        return results_list
