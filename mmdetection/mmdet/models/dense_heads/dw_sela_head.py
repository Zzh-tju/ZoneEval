...
class CenterPrior(nn.Module):
    ...
    def forward(self, anchor_points_list, gt_bboxes, labels,
        ...
        for slvl_points, stride in zip(anchor_points_list, self.strides):
            ...
            x = gt_center_x
            y = gt_center_y
            x_weight = torch.abs((1/w)*(x-0.5*w))
            y_weight = torch.abs((1/h)*(y-0.5*h))
            weight,_ = torch.max(torch.stack((x_weight*2,y_weight*2),dim=0),dim=0)
            ...
            center_prior = center_prior * (self.gamma*weight + 1)
            center_prior_list.append(center_prior)
            del weight, x,y,x_weight,y_weight
...
class DWSELAHead(AnchorFreeHead):
    def __init__(self,
                 *args,
                 soft_prior=True,
                 reg_refine=True,
    ...
    def init_weights(self):
        super(DWHeadSELA, self).init_weights()
    ...
    def forward_single(self, x, scale, stride):
        ...
        if self.with_reg_refine:
            ...
            bbox_pred = self.relu(bbox2distance(points, bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)).reshape(b, h, w, 4).permute(0, 3, 1, 2).contiguous())
    ...
    def loss(self,
        ...
        for gt_bboxe, gt_label, inside_gt_bbox_mask,img_meta in zip(gt_bboxes, gt_labels, inside_gt_bbox_mask_list,img_metas):
            center_prior_weight, inside_gt_bbox_mask = self.center_prior(all_level_points, gt_bboxe, gt_label, inside_gt_bbox_mask,img_meta["img_shape"])
