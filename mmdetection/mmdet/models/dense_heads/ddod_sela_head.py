...
from mmdet.core import (anchor_inside_flags, build_assigner, build_sampler, bbox_xyxy_to_cxcywh,
...
class DDODSELAHead(AnchorHead):
    ...
    def loss_cls_single(self, cls_score, labels, label_weights, spatial_weights,
        ...
        label_weights = label_weights * (self.gamma*spatial_weights+1)
        loss_cls = self.loss_cls(
            cls_score, labels, label_weights, avg_factor=num_total_samples)
        return reweight_factor * loss_cls,

    def loss_reg_single(self, anchors, bbox_pred, iou_pred, labels,
                        label_weights, bbox_targets, bbox_weights, spatial_weights,
                        reweight_factor, num_total_samples):
        ...
        if len(pos_inds) > 0:
            ...
            pos_spatial_weights = self.gamma * spatial_weights[pos_inds] + 1
            # regression loss
            loss_bbox = self.loss_bbox(
                pos_decode_bbox_pred,
                pos_decode_bbox_targets,
                weight=pos_spatial_weights,
    ...
    def loss(self,
        ...
        (cls_anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, spatial_weights_list, num_total_pos, num_total_neg) = cls_targets
        ...
        (reg_anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, spatial_weights_list, num_total_pos, num_total_neg) = reg_targets
    ...
    def get_cls_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, all_spatial_weights, pos_inds_list, neg_inds_list) = multi_apply(
        ...
        bbox_weights_list = images_to_levels(all_bbox_weights,
                                             num_level_anchors_list[0])
        spatial_weights_list = images_to_levels(all_spatial_weights,
                                             num_level_anchors_list[0])

        return (anchors_list, labels_list, label_weights_list,
                bbox_targets_list, bbox_weights_list, spatial_weights_list, num_total_pos,
                num_total_neg)
    ...
    def _get_target_single(self,
        ...
        assign_result = assigner.assign(anchors, num_level_anchors_inside,
                                        ...
                                        cls_scores_valid, bbox_preds_valid, img_w, img_h)
        ...
        if len(pos_inds) > 0:
            ...
            spatial_weights[pos_inds] = 2*torch.max(torch.abs(xywh_anchors[pos_inds, 0] - 0.5*img_w)/img_w, torch.abs(xywh_anchors[pos_inds, 1] - 0.5*img_h)/img_h)
        ...
        if unmap_outputs:
            ...
            spatial_weights = unmap(spatial_weights, num_total_anchors, inside_flags)
            bbox_targets = unmap(bbox_targets, num_total_anchors, inside_flags)
            bbox_weights = unmap(bbox_weights, num_total_anchors, inside_flags)

        return (anchors, labels, label_weights, bbox_targets, bbox_weights, spatial_weights,
                pos_inds, neg_inds)
